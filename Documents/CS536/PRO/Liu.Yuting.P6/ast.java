...
class ProgramNode extends ASTnode {
     ...
     */
    public void nameAnalysis(int offset) {
        SymTable symTab = new SymTable();
        myDeclList.nameAnalysis(symTab, offset, true);
     ...
     * code Generator
     */
    public void codeGenerator(PrintWriter p) {
        myDeclList.codeGenerator(p);
    }
...
class DeclListNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, int offset, boolean isGlobal) {
            nameAnalysis(symTab, symTab, offset, isGlobal);
    }
     ...
     * decls in the list.
     */    
    public void nameAnalysis(SymTable symTab, SymTable globalTab, int offset, boolean isGlobal) {
        if(isGlobal) {
...
class FormalsListNode extends ASTnode {
    ...
    public List<Type> nameAnalysis(SymTable symTab, int offset) {
        ...
        for (int i = myFormals.size() - 1; i >= 0; --i) {
            SemSym sym = myFormals.get(i).nameAnalysis(symTab, offset);
            offset += 4;
            if (sym != null) {
    ...
    public void codeGenerator(PrintWriter p, String label) {
        // main function
        if(label.equals("main")) {
            for(int i = 0; i < myFormals.size(); ++i) {
                // initialize:
                // push formals with value 0
...
class FnBodyNode extends ASTnode {
    ...
    public void nameAnalysis(SymTable symTab, int offset) {
        ...
        myStmtList.nameAnalysis(symTab, offset - 4 * myDeclList.getListSize());
    }    
    ...
    public void codeGenerator(PrintWriter p, String label) {
        myDeclList.codeGenerator(p);
        myStmtList.codeGenerator(p, label);
    }
...
class VarDeclNode extends DeclNode {
     ...
     */
    public void codeGenerator(PrintWriter p) {
        // not struct variable declaration
        if(!(myType instanceof StructNode)) {
            // local variable
            if(myId.sym().getOffset() < 0) {
                // initialize local variable = 0 when declaration
                // push into the stack with offset of $fp
                Codegen.genPush(Codegen.ZERO);
            }
            // global variable
            else if(myId.sym().getOffset() == 0) {
                p.println('\t' + ".data");
                // initialize global variable = 0 when declaration
...
class FnDeclNode extends DeclNode {
    ...
    public SemSym nameAnalysis(SymTable symTab, int offset) {
        ...
        myBody.nameAnalysis(symTab, offset - 8); // process the function body
    ...
    */
    public void codeGenerator(PrintWriter p) {
        ...
        if(isMain) {
            ...
            Codegen.generateLabeled(myId.name(), "", "MAIN ENTRY");
            Codegen.genLabel("__start", "");
            if(myFormalsList.length() != 0) {
                myFormalsList.codeGenerator(p, myId.name());
            }
        }
        else {
            // generate label for non-main function
            Codegen.genLabel("_" + myId.name(), "FUNC DECL");
        }
        // push $ra
        ...
        // code gen for mybody
        myBody.codeGenerator(p, retLabel);
        
        // function exit
        Codegen.genLabel(retLabel);
        // tear up
        Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP, 0, "FUNC EPILOGUE");
        ...
        // main function: system calls
        if(myId.name().equals("main")) {
            Codegen.generate("li", Codegen.V0, "10");
            Codegen.generateWithComment("syscall", "MAIN RETURN SYS CALL");
        }
        // other function exit
        else {
...
class StructDeclNode extends DeclNode {
    ...
    public SemSym nameAnalysis(SymTable symTab, int offset) {
        ...
        myDeclList.nameAnalysis(structSymTab, symTab, offset, false);
...
abstract class StmtNode extends ASTnode {
    ...
    abstract public void codeGenerator(PrintWriter p, String label);
}
...
class WriteStmtNode extends StmtNode {
    ...
    public void codeGenerator(PrintWriter p, String label) {
        ...
        } else if(myExp.typeCheck().isIntType() || myExp.typeCheck().isBoolType()){
            Codegen.generate("li", Codegen.V0, "1");
        }
        Codegen.generateWithComment("syscall", "WRITE RETURN");
    }
...
class IfStmtNode extends StmtNode {
    ...
    public void codeGenerator(PrintWriter p, String label) {
        ...
        String if_end = Codegen.nextLabel();
        Codegen.generate("beq", Codegen.T0, Codegen.ZERO, if_end);
        myDeclList.codeGenerator(p);
        myStmtList.codeGenerator(p, label);
        
        Codegen.generateLabeled(if_end, "nop", "");
    }
...
class IfElseStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab, int offset) {
        ...
        offset -= 4 * myThenDeclList.getListSize();
        myThenStmtList.nameAnalysis(symTab, offset);
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
            System.err.println("Unexpected EmptySymTableException " +
        ...
        myElseStmtList.nameAnalysis(symTab, offset - 4 * myElseDeclList.getListSize());
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
            System.err.println("Unexpected EmptySymTableException " +
    ...
    public void codeGenerator(PrintWriter p, String label) {
        ...
        // get label name
        String if_end = Codegen.nextLabel();
        String else_end = Codegen.nextLabel();
        
        // condition is true - if:
        Codegen.generate("beq", Codegen.T0, Codegen.ZERO, if_end);
        myThenDeclList.codeGenerator(p);
        myThenStmtList.codeGenerator(p, label);
        Codegen.generate("j", else_end);
        
        // else:
        Codegen.genLabel(if_end);
        myElseDeclList.codeGenerator(p);
        myElseStmtList.codeGenerator(p, label);
        
        Codegen.generateLabeled(else_end, "nop", "");
    }
...
class WhileStmtNode extends StmtNode {
    ...
    public void codeGenerator(PrintWriter p, String label) {
        ...
        // condition is false
        Codegen.generate("beq", Codegen.T0, Codegen.ZERO, label_exit);
        myDeclList.codeGenerator(p);
        myStmtList.codeGenerator(p, label);
        // loop back
...
abstract class ExpNode extends ASTnode {
    ...
    abstract public void codeGenerator(PrintWriter p);
}
...
class AssignNode extends ExpNode {
     ...
     */
    public void codeGenerator(PrintWriter p) {
        // get r-value of Rhs into T0
        myExp.codeGenerator(p);
        Codegen.genPop(Codegen.T0);
        
        // write r-value on the place with l-value addr
        // LHS can only be IdNode
        int offset = ((IdNode)myLhs).sym().getOffset();
        // global variable
        if(offset == 0) {
            Codegen.generateWithComment("sw", "LHS ASSIGNED", Codegen.T0, "." + ((IdNode)myLhs).name());
        }
        else {
            Codegen.generateIndexed("sw", Codegen.T0, Codegen.FP, ((IdNode)myLhs).sym().getOffset(), "LHS ASSIGNED");
        }
...
class UnaryMinusNode extends UnaryExpNode {
    ...
    public void codeGenerator(PrintWriter p) {
        ...
        Codegen.genPop(Codegen.T0);
        // 0 - exp
        Codegen.generate("sub", Codegen.T0, Codegen.ZERO, Codegen.T0);
        Codegen.genPush(Codegen.T0);
    }
...
class PlusNode extends ArithmeticExpNode {
     ...
     */
    public void codeGenerator(PrintWriter p) {
        myExp1.codeGenerator(p);
        myExp2.codeGenerator(p);
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGenerator(PrintWriter p) {
        ...
        // lhs is false --> break
        String label_rhs = Codegen.nextLabel();
        String label_res = Codegen.nextLabel();
        Codegen.generate("beq", Codegen.T0, Codegen.ZERO, label_res);
        
        Codegen.genLabel(label_rhs);
        myExp2.codeGenerator(p);
        Codegen.genPop(Codegen.T0);
        
        Codegen.genLabel(label_res);
        Codegen.genPush(Codegen.T0);
    }
...
class OrNode extends LogicalExpNode {
    ...
    public void codeGenerator(PrintWriter p) {
        ...
        String label_rhs = Codegen.nextLabel();
        String label_res = Codegen.nextLabel();
        Codegen.generate("bne", Codegen.T0, Codegen.ZERO, label_res);
